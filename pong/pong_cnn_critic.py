...
dst_weight = critic.state_dict()
for key in dst_weight:
    if key.startswith('fc.0.weight'):
        dst_weight[key][:, :-1] = critic_weight[key]
    else:
        dst_weight[key] = critic_weight[key]
critic.load_state_dict(dst_weight)
...
for i in range(ckpt, num_episodes):
    ...
    cnts = torch.tensor(cnts, dtype=torch.float32).view(-1, 1).to(device)
    cnts = cnts / 200
    values = critic(states, cnts)
    # print(values)
