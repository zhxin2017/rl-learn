...
class Agent:
    def __init__(self, model=None, rec_file='rec.json', state_capacity=500000, device=torch.device('mps')):
    ...
    def kill(self, board: Board):
        pieces, feasible_moves = board.get_feasible_moves()
        n_piece = len(pieces)
    ...
    def self_play(self, board: Board, show_board, game_uuid, num_simulation=500):
